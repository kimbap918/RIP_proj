...
def category(request, pk):
    ...
    # 추천순
    if sort == '1':
        article = Article.objects.filter(category=pk).annotate(like_cnt=Count('like_user')).order_by('-like_cnt')
    # 답글많은 순
    elif sort == '2':
        article = Article.objects.filter(category=pk).annotate(comments=Count('comment')).order_by('-comments')
    # 내가 쓴 글
    elif sort == '3':
        user = get_user_model().objects.get(pk=request.user.pk)
        article = user.article_set.all()
